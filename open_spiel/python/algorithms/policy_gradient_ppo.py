...
from open_spiel.python.algorithms.losses import rl_losses, rl_losses_ppo
...
Transition_ppo = collections.namedtuple(
    "Transition", "info_state action reward discount legal_actions_mask old_prob_a values")
...
class PolicyGradient(rl_agent.AbstractAgent):
  ...
  def __init__(self,
    ...
    self._old_prob_a = tf.placeholder(
        shape=[None], dtype=tf.float32, name="old_prob_a_ph")
    self._legal_actions_mask = tf.placeholder(
        shape=[None, num_actions], dtype=tf.float32, name="legal_actions_mask_ph")
    self._advantage = tf.placeholder(
        shape=[None], dtype=tf.float32, name="advantage_ph")
    ...
    if loss_class.__name__ == "BatchA2CLoss" or loss_class.__name__ == "BatchPPOLoss":
    ...
    if optimizer_str == "adam":
      ...
      self._critic_optimizer = tf.train.RMSPropOptimizer(
          learning_rate=critic_learning_rate)
    ...
    elif loss_class.__name__ == "BatchPPOLoss":
      self._pi_loss = pg_class.loss(
          policy_logits=self._policy_logits,
          baseline=self._baseline,
          actions=self._action_ph,
          returns=self._return_ph,
          old_prob_a=self._old_prob_a,
    ...
    if optimizer_str == "adam":
      ...
      self._pi_optimizer = tf.train.RMSPropOptimizer(
          learning_rate=pi_learning_rate)
    elif optimizer_str == "sgd":
  ...
  def _get_loss_class(self, loss_str):
    if loss_str == "rpg":
      ...
      return rl_losses_ppo.BatchPPOLoss

  def _act(self, info_state, legal_actions):
    # Make a singleton batch for NN compatibility: [1, info_state_size]
    info_state = np.reshape(info_state, [1, -1])
    policy_probs, values = self._session.run(
        [self._policy_probs, self._baseline],
  ...
  def _add_episode_data_to_dataset(self):
    ...
    for t in reversed(range(len(rewards))):
        if t == len(rewards) - 1:  # 数据末状态
            curnonterminal = 0  # 数据末状态  游戏是否未结束
            nextvalues = 0  # 数据末状态  预测价值
        else:
            curnonterminal = 1.0
  ...
  def get_weights(self):
    ...
    if self._loss_class.__name__ == "BatchA2CLoss" or self._loss_class.__name__ == "BatchPPOLoss":
